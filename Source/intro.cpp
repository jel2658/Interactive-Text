...
void emscripten_notes() {
	htmlOutput(R"(Interactive Text is just output and formatting. Emscripten does the real work converting C++ to Javascript. Emscripten has some catches.
...
Add '\n' to your console output messages, or they won't show up. <a href="https://floooh.github.io/2016/08/27/asmjs-diet.html">Don't use iostreams; they are expensive</a>, 250KB.
	...
	htmlOutput("Return", engine_choice, n)(r);
}

void other_engines() {
	htmlOutput(R"(Java has severe UI and performance problems in every text game I've seen.
...
void engine_choice() {
	htmlOutput(R"(Technical details:
<ul style=margin:0>
...
Both options create webpages as output.
	...
	htmlOutput("Other engines", other_engines)('\n');
	htmlOutput("Notes on the compiler", emscripten_notes)('\n');
	htmlOutput("Return to the main menu", main_menu_later, n)(r);
}

int incrementor = 299; //odd = erased, even = draw
void guestbook_incrementor() {
	htmlOutput("You doodle on the board.");
	incrementor = (incrementor) / 2 * 2 + 2;
	main_menu_later();
}
void erase_incrementor() {
	htmlOutput("You wipe the board clean.\n");
	++incrementor;
	main_menu_later();
}
...
void main_menu_later() {
	...
	if (!(incrementor & 1)) {
		htmlOutput("Draw on it again?", guestbook_incrementor)("Or")("erase the board?", erase_incrementor);
		htmlOutput(R"(<div style="text-align:center;"><img src="https://picsum.photos/)")(incrementor, ns)(R"(/200/?random" alt=doodle style="width:)")(incrementor, ns)(R"(px; height:200px"></div>
)");
	}
	htmlOutput(R"(Interactive Text turns C++ text games into HTML. Features:
<ul style=margin:0>
...
<li>this webpage was built with it</li>
	...
	htmlOutput("Further details", engine_choice)('\n');

	if (incrementor & 1) htmlOutput("In front of you is a board.")("Draw on it?", guestbook_incrementor)('\n');
	htmlOutput(R"(<a href="https://github.com/ad8e/Interactive-Text">Download the engine.</a>)");
	htmlOutput(r);
}
